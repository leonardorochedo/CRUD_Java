...
public class CarDao implements DaoInterface {
    ...
    public void insertElement(Car car) {
        ...
        try {
            String sqlScript = "INSERT INTO Carros (modelo, ano, preco) VALUES (?, ?, ?)";
    ...
    public void refreshElement(Car car) {
        ...
        try {
            String sqlScript = "UPDATE Carros SET modelo = ?, ano = ?, preco = ? WHERE id = ?";
            comand = connection.prepareStatement(sqlScript);
            comand.setString(1, car.getModel()); // Modelo
            comand.setInt(2, car.getYear()); // Ano
            comand.setDouble(3, car.getPrice()); // Preco
    ...
    public void removeElement(Car car) {
        // Remover elemento
        try {
            String sqlScript = "DELETE FROM Carros WHERE Carros.id = ?";
            comand = connection.prepareStatement(sqlScript);
    ...
    public Car searchId(int id) {
        ...
        String sqlScript = "SELECT * FROM Carros WHERE Car.id = ?";
        try {
            comand = connection.prepareStatement(sqlScript);
            comand.setInt(1, id);
    ...
    public List<Car> getAll() {
        // GET em todos os elementos de Carros
        String sqlScript = "SELECT * FROM Carros";
